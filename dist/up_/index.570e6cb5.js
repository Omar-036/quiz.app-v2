$(function() {
    ...
    function showSearchResult(speed) {
        ...
        $(".material__name").each(function(i, el) {
            if ($(this).text().toLocaleLowerCase().indexOf(inputValue) > -1) $(this).parent().show(speed);
            else {
        ...
        if ($(sectionMaterials).children().not("h2").length - 1 == numberOfMaterialHide) $(".not-found").show(speed);
    }
    ////////////////////////
    // ACTIVE SEARCH BOX
    let inputValue;
    $(searchInput).keyup(function(e) {
        ...
        if (inputValue) {
            ...
            emptyArray = data.filter((d)=>d.toLocaleLowerCase().indexOf(inputValue.toLocaleLowerCase()) > -1);
            emptyArray = emptyArray.map((d, i)=>`<li class='auto-complete__list'>${d}</li>`);
...
//# sourceMappingURL=index.570e6cb5.js.map
