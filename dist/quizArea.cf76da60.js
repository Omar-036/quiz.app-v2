$(function() {
    ...
    $.getJSON("../json/quiz-data.json", function(data) {
        const quizMaterial = data[localStorage.getItem("quizIndex")][localStorage.getItem("quizNumber")];
        let numberOfQuestions = quizMaterial["quiz-data"].length;
        $(document).prop("title", "Test Bank | " + textCapitalizeFormat(quizMaterial["quizName"]));
        ...
        if (alert.length) $(alert).each(function(i, mes) {
            const alertBox = `
      <div class="alert">
      ...
      `;
        ...
        $(quizMaterial["quiz-data"]).each(function(index, question) {
            ...
            const questionTitleText = `<h3 class="quiz-data__title--text"><span>${question["answers"].length !== 0 ? String(curIndex + 1).length > 1 ? curIndex += 1 : "0" + (curIndex += 1) + " - " : ""} </span>${textCapitalizeFormat(question["titleText"])}</h3>`;
            ...
            $(question["answers"]).each(function(index, answer) {
                let answerLi;
                if (answer === question.rightAnswer[0]) answerLi = `<li class="quiz-data__answer pwd quiz-data__answer--${index + 1}">${answer}</li>`;
                else answerLi = `<li class="quiz-data__answer quiz-data__answer--${index + 1}">${answer}</li>`;
                $(".quiz-data__answers").last().append(answerLi);
                $(".quiz-data__answer").last().on("click", function(answer) {
                    ...
                    $(answer.target).parent().parent().siblings(".quiz-data__desc").children(".show-more").css("display", "flex");
                    ...
                    if (numberOfQuestions === rightAnswers + wrongAnswers) {
                        ...
                        $(".quiz__next-step a").on("click", function() {
                            if (this.classList.contains("pre")) {
                                if (localStorage.getItem("quizNumber") > 0) localStorage.setItem("quizNumber", Number(localStorage.getItem("quizNumber")) - 1);
                                else {
                                    $(this).removeAttr("href");
                                    setPopupText(quizMaterial["quiz-type"], "There is no previous exam", "لا يوجد أمتحان سابق");
                                    $(".popup-main").fadeIn(150);
                                    setTimeout(function() {
                                        $(".popup-main").fadeOut(350);
                                    }, 1000);
                                ...
                                if (+localStorage.getItem("quizNumber") < +data[localStorage.getItem("quizIndex")].length - 1) localStorage.setItem("quizNumber", Number(localStorage.getItem("quizNumber")) + 1);
                                else {
                                    $(this).removeAttr("href");
                                    setPopupText(quizMaterial["quiz-type"], "Sorry, this was the last exam", "عذراََ, كان هذا الأمتحان الأخير");
                                    $(".popup-main").fadeIn(150);
                                    setTimeout(function() {
                                        $(".popup-main").fadeOut(350);
                                    }, 1000);
            ...
            if (question.hasDescription) {
             ...
             <span><i class="fa-solid fa-arrow-right"></i></span>
                ...
                if (quizMaterial["quiz-type"] === "english") $(".show-more").last().html("Read Description<span class='show-more--english'><i class='fa-solid fa-arrow-right'></i></span>");
                else $(".show-more").last().html("قراءة الوصف<span class='show-more--arabic'><i class='fa-solid fa-arrow-left'></i></span>");
                if (question?.video?.length > 0) $(question.video).each(function(index, video) {
                    const descVideo = `<video class="quiz-data__video" width="100%" ${video.autoplay ? "autoplay" : ""} ${video.muted ? "muted" : ""} ${video.control ? "control" : ""} ${video.loop ? "loop" : ""}>
        ...
        Your browser does not support the video tag.
                ...
                if (question.image.length > 0) $(question.image).each(function(index, image) {
                ...
                if (question.text.length > 0) $(question.text).each(function(index, image) {
        ...
        function appendQuizResult() {
            //
            let nextStep = "";
            if (quizMaterial["quiz-type"] === "english") nextStep = `
    ...
    `;
            let quizResult = "";
            if (quizMaterial["quiz-type"] === "english") quizResult = `
    ...
    function greeterText(typeOfQuiz, rightAnswers, totalMarks) {
        if (typeOfQuiz === "english") {
            if (rightAnswers === totalMarks) return `Perfect<span> 👏 </span>`;
            else if (rightAnswers >= totalMarks / 2) return `Good!<span> 🙄 </span>`;
            ...
            if (rightAnswers === totalMarks) return `ما شاء الله عليك<span> 👏 </span>`;
            else if (rightAnswers >= totalMarks / 2) return `علامتك جيدة<span> 🙄 </span>`;
            else if (rightAnswers < totalMarks) {
                setTimeout(function() {
                    $(".re").addClass("active");
                }, 2000);
...
//# sourceMappingURL=quizArea.cf76da60.js.map
